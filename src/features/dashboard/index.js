...
function Dashboard() {
    ...
    useEffect(()=>{
        dispatch(getEvent({page:currentPage , role:currentUser?.role}))
        if(!currentUser){
    ...
    return (
        <>
            ...
            {showModalEdit && <ModalEditinfo onClose={closeModal} onSave={updateDashboardPeriod} eventID={selectedID} userDetails={selectedUserDetails}/>}
            {isModalOpen && <ModalCA onClose={closeModal} onSave={updateDashboardPeriod} />}
            <TitleCard2 title={'ประเภทกิจกรรม'} title2={roleSelect} subTitle={`ทั้งหมด ${eventData.meta?.total} รายการ`} TopSideButtons={createBTN}>
